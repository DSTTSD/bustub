...
inline uint32_t HASH_TABLE_TYPE::KeyToPageId(KeyType key, HashTableDirectoryPage *dir_page) {
  // 把hash转化为dir_index, 获取对应bucket的pageid,调用现有函数即可
...
HashTableDirectoryPage *HASH_TABLE_TYPE::FetchDirectoryPage() {
  ...
  if (directory_page_id_ == INVALID_PAGE_ID) {
    ...
    Page *page = buffer_pool_manager_->NewPage(&new_page_id_dir); // 这里有进行memset
    assert(page != nullptr);
...
bool HASH_TABLE_TYPE::GetValue(Transaction *transaction, const KeyType &key, std::vector<ValueType> *result) {
  ...
  //1 获取directory table, 继而获得对应busket
...
bool HASH_TABLE_TYPE::SplitInsert(Transaction *transaction, const KeyType &key, const ValueType &value) {
  ...
  MappingType *origin_array = split_bucket->GetArrayCopy(); // new array将原来的内容保存
  split_bucket->Reset();
  
  //3.2 创建一个image bucket，并初始化该image bucket(兄弟bucket)
  ...
  dir_page->SetBucketPageId(split_image_bucket_index, image_bucket_page_id); // 修改

  // 3.3 对于原来bucket中每个数据，分配，根据image还是原来的bucket
  for (uint32_t i = 0; i < origin_array_size; i++) {
    // target_bucket_index : hash(key) & 
  ...
  // 最难的部分：为什么这样设置同一级 
  //将所有同一级（第i（localdepth）位为1或者0开头的所有）的bucket设置为相同的local depth和page
...
/*****************************************************************************
 * MERGE
 target_bucket_index代表了已经为空的bucket
 *****************************************************************************/
template <typename KeyType, typename ValueType, typename KeyComparator>
